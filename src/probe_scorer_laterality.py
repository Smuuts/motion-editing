...
The pre-gate for taking an edit's localisation from an external frozen scorer instead of
from this backbone, whose masks are measured to have no laterality at all
(instruction-invariance r 0.985, net laterality response ~0 on the converged x0
checkpoint). That whole family rests on one untested assumption — that the borrowed
scorer knows something our denoiser does not. If it does not, the route dies here for
half a day's compute, and "laterality is absent from motion-language embeddings, not just
from our model" becomes a stronger claim than the in-house negative it replaces.

No checkpoint of ours is involved: this measures the scorer, not the editor.

WHY THE OBVIOUS EXPERIMENT IS A TRAP. Scoring one clip against "…left leg" vs "…right
leg" and seeing "left" win proves nothing: a scorer with a constant preference for the
word "left" does that on every clip, including clips that kick right. Both parts below
are therefore built as FORCED CHOICES A CONSTANT BIAS CANNOT WIN (chance is exactly
50 %), and every accuracy is reported with a binomial CI and split by side.

Part A — mirrored caption x mirrored motion, fully in-distribution and tagger-free.
  HumanML3D ships an `M`-prefixed mirror of every clip whose caption is the left/right
  word-swap of the original, and leaves captions without a laterality word
  byte-identical — which both supplies the minimal pairs and filters them. For motions
  {x, Mx} x captions {c, Mc}, count two forced choices:
...
Part B — instruction antisymmetry, on the short imperative strings the editor sends.
  Needs no ground-truth side label: mirroring the motion must flip the preference.
      d(x)  = s(x,  "…left …") - s(x,  "…right …")
      d(Mx) = s(Mx, "…left …") - s(Mx, "…right …")
  A laterality-capable scorer gives sign d(x) != sign d(Mx). Reported as the sign-flip
  rate plus the effect size |d| against the scorer's own score spread.

Controls — an unrelated-caption arm (must be near-ceiling, else the plumbing is broken
and nothing else means anything), a per-side split (catches a constant side bias), a
tagger check against the tokens on disk, and an embedding-level check that our tagging of
a caption lands where the disk tokens land.
"""
...
from analysis.laterality_report import print_report
...
def main():
    ...
    log.info(f"Lateralised mirrored pairs in {args.split}: {len(pairs)}")

    scorer = build_scorer(args.scorer, evaluator_dir=args.evaluator_dir, device=device)

    log.info("Embedding motions …")
