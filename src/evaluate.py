...
def mpjpe_over_clips(clips, to_joints, smooth_sigma):
    ...
    values = []
    for clip in log.progress(clips, desc="MPJPE", leave=True):
...
def main():
    ...
    log.info(f"  {N} clips")
    if N == 0:
        log.info("No clips found — nothing to report.")
        return
    ...
    log.section("MPJPE (internal, not comparable to literature)")
    mpjpe = mpjpe_over_clips(clips, to_joints, args.smooth_sigma)
    log.info(f"MPJPE   {mpjpe.mean()*1000:.2f} ± {mpjpe.std()*1000:.2f} mm  (N={N})")

    log.section("Encoding motions and texts")
    gen_embs = evaluator.encode_motion(
        log.progress([to_eval_norm(c["gen_norm"]) for c in clips],
                     desc="  gen motions", leave=True))
    gt_embs = evaluator.encode_motion(
        log.progress([to_eval_norm(c["gt_norm"]) for c in clips],
                     desc="  gt  motions", leave=True))
    text_embs = evaluator.encode_text([c["tokens"] for c in clips])
    log.info(f"  embedding dim: {gen_embs.shape[1]}")
